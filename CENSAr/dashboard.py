...
st.set_page_config(
    page_title="CENSAr",
    page_icon=f"{PARENT_DIR}/sl//favicon.ico",
    layout="wide",
...
# CSS
with open(f"{PARENT_DIR}/sl/style.css") as f:
...
if menu_list == "Inicio":
    ...
    landing = Image.open(f"{PARENT_DIR}/img/tejido_urbano.png")
    ...
    * **Poblacion**:
    ...
    st.markdown(
        "Seleccione una variable censal para analizar su distribución en la región deseada"
    )
    st.markdown(" ")
    ...
    indicators = [
        ...
        "Viviendas en altura",
        "Viviendas en áreas de difícil acceso",
    ]
    ...
    if (indicator == "Calidad constructiva de la vivienda") and (
        region == "Capital Federal"
    ):
        area_superior = col3.selectbox("Area superior", ["barrio", "comuna"])
        categoria_inmat = col4.selectbox(
            "Categoría", ["recuperables", "irrecuperables", "aceptables"]
        )

        radios_geom = radios_caba_2010()
        radios_vals = inmat_radios_caba_2010()

    elif (indicator == "Calidad constructiva de la vivienda") and (
        region != "Capital Federal"
    ):
        area_superior = col3.selectbox("Area superior", ["departamento"])
        categoria_inmat = col4.selectbox(
            "Categoría", ["aceptables", "recuperables", "irrecuperables"]
        )
    ...
    inmat_inf = radios_inmat_2010(
        region_name=region, geog=radios_geom, vals=radios_vals
    )

    col5, col6 = st.columns((1, 1))
    fig1 = construye_territorio(
        gdf=inmat_inf,
        nombre_unidad_s=area_superior,
        nombre_unidad_i="str_link",
        nombre_variable="total_inmat",
        nombre_categoria=categoria_inmat,
        estadistico="CEC",
        tipo="bar",
        dinamico=True,
    )

    fig2 = construye_territorio(
        gdf=inmat_inf,
        nombre_unidad_s=area_superior,
        nombre_unidad_i="str_link",
        nombre_variable="total_inmat",
    ...
    territorio_superior = construye_territorio(
        gdf=inmat_inf,
        nombre_unidad_s=area_superior,
        nombre_unidad_i="str_link",
        nombre_variable="total_inmat",
    ...
    inmat_sup = indice_geografia_superior(
        territorio_df=territorio_superior,
        nombre_area=area_superior,
        nombre_region=region,
    )
    fig3 = plot_folium_dual_choroplet(
        gdf_inferior=inmat_inf,
        gdf_superior=inmat_sup,
        categoria=categoria_inmat,
        indicador_superior="CEC",
        nombre_superior=area_superior,
        nombre_region=region,
    )
