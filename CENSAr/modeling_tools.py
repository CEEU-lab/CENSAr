...
from datasources import tracts_matching_0110

def totals_forecast(gdf_pers_01, gdf_var_01, gdf_pers_10, gdf_var_10,
                    proyections_df, namedept, base_year, forecast_year):
  """
...
def distribute_totals_tract(tot_var, weights, catname, 
                            forecast_year, gdf):
  """
  Returns the projected number of households or residential units
  ...
  if weights is None:
    ...
    gdf_reset[f'{catname}_dist'] = gdf_reset[f'{catname}']/gdf_reset[f'{catname}'].sum()
    weights = gdf_reset[f'{catname}_dist'].fillna(0)/gdf_reset[f'{catname}_dist'].sum()

  np.random.seed(1)
  drawn_tracts = choice(gdf_reset['link'], 
                        tot_var,  
                        p=weights 
                        )
  ...
  gdf_reset[f'{catname}_{forecast_year}'] = gdf_reset['link'].map(proj_totals).fillna(0)
  totals_by_tract = dict(zip(gdf_reset['link'],gdf_reset[f'{catname}_{forecast_year}']))
...
def var_forecast(gdf_2001, gdf_2010, catname, 
                 gdf_2020, pct_target, base_year, 
                 tot_colname):
  """
  ...
  if base_year == '2001':
    dist_dict = observed_dist(catname=catname, idx_col=idx_col, base_year='2001', 
                              gdf_base=gdf_2001, gdf_forecast=gdf_2020) # 2001 distribution
    gdf_2020[f'{catname}_2001'] = gdf_2020[idx_col].map(dist_dict) 
    dist_var = gdf_2020[f'{catname}_2001']

  elif base_year == '2010':
    dist_dict = observed_dist(catname=catname, idx_col=idx_col, base_year='2010', 
                              gdf_base=gdf_2010, gdf_forecast=gdf_2020) # 2010 distribution
    ...
    for year in ['2001','2010']:
      ...
      dist_dict = observed_dist(catname=catname, idx_col=idx_col, base_year=year, 
                                gdf_base=data[year], gdf_forecast=gdf_2020) 
      gdf_2020[f'var_{year}'] = gdf_2020[idx_col].map(dist_dict)
    
    gdf_2020['var_0110'] = round((gdf_2020['var_2001'] + gdf_2020['var_2010'])/2, 4) # middle beetwen 01 and 10
    dist_var = gdf_2020[f'var_0110']
    
  totcat = int(gdf_2020[tot_colname].sum()*pct_target/100) # total number to be distributed by tract
  weights = dist_var.fillna(0)/dist_var.sum()
  
  return totcat, weights

def simulate_total_var(gdf_pers_01, gdf_var_01, 
                       gdf_pers_10, gdf_var_10,
                       proy_df, namedept, 
  ...
  proj_total = totals_forecast(gdf_pers_01, gdf_var_01, 
                               gdf_pers_10, gdf_var_10,
                               proy_df, namedept, 
                               base_year, forecast_year)
  ...
  sim_dist = distribute_totals_tract(tot_var=proj_total, weights=None, catname=catname, 
                                     forecast_year=forecast_year, gdf=gdf_var_10)
  return sim_dist

def simulate_cat_var(gdf_var_01, gdf_var_10,
                     base_year, forecast_year, 
                     forecast_gdf, pct_val, 
                     catname, tot_colname):
  """
  ...
  cat_var, probs = var_forecast(gdf_2001=gdf_var_01, gdf_2010=gdf_var_10, catname=catname, 
                                  gdf_2020=forecast_gdf, pct_target=pct_val, base_year=base_year,
                                  tot_colname = tot_colname)
  
  sim_dist = distribute_totals_tract(tot_var=cat_var, weights=probs, catname=None, 
                                     forecast_year=forecast_year, gdf=forecast_gdf)
  return sim_dist
...
def tracts_2020_to_2010(tracts_2020_gdf, tracts_2010_gdf):
  ...
  tracts_20_to_10 = gpd.sjoin(tracts_2020_gdf_rep, tracts_2010_gdf, predicate='within')
  tracts_20_to_10.rename(columns={'link_left':'link_2020', 'link_right':'link_2010'}, inplace=True)
  ...
  tracts_missing.rename(columns={'link_left':'link_2020', 'link_right':'link_2010_'}, inplace=True)
  tracts_nearest = dict(zip(tracts_missing.link_2020, tracts_missing.link_2010_))

  tracts_2020_gdf['link_2010'].fillna(tracts_2020_gdf['link'].map(tracts_nearest), inplace=True)
...
def tracts_2010_to_2001(tracts_2020_gdf):
  ...
  df = tracts_matching_0110(prov='corrientes', 
